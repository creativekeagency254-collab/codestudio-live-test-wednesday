...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: invoices } = await supabase
      .from("invoices")
      .select("id, amount, currency, status, project_id, client_id, client_email, created_by, invoice_type, paystack_reference")
      ...
      if (isSuccessWebhookEvent(normalizedEventType, eventData)) {
        ...
        const paymentInsert = await supabase
          ...
          .upsert({
            ...
            status: "paid",
            amount: Number(invoice.amount || amountFromPaystackKobo(eventData.amount) || 0),
        ...
        if (invoiceType === "deposit" && invoice.project_id) {
          ...
          if (projectRow?.id) {
            ...
            const depositAmount = Number(invoice.amount || amountFromPaystackKobo(eventData.amount) || 0);
        ...
        if (clientProfileId) {
          const notification = await supabase.from("notifications").insert({
            ...
            body: invoiceType === "deposit"
              ? "Deposit received successfully. Your project has moved forward."
              : "Your payment was successful. Project workflow continues.",
            payload: {
              invoice_id: invoice.id,
